"use client";

import { useMemo, useState } from "react";
import {
  SCHOOL_CCA_KIT_BOND_LAST_REVIEWED,
  estimateSchoolCcaKitBond,
  formatSgd,
  type CcaKitBondMode,
} from "@/lib/tools/school-cca-kit-bond";

const MODES: Array<{ id: CcaKitBondMode; label: string }> = [
  { id: "full-return", label: "Full return" },
  { id: "damage-hold", label: "Damage hold" },
  { id: "lost-forfeit", label: "Lost / forfeit" },
];

export function SchoolCcaKitBondCalculator() {
  const [mode, setMode] = useState<CcaKitBondMode>("full-return");
  const [bondSgd, setBondSgd] = useState(400);
  const [overdueDays, setOverdueDays] = useState(0);
  const [overduePerDaySgd, setOverduePerDaySgd] = useState(5);
  const [damageSgd, setDamageSgd] = useState(80);
  const [adminFeeSgd, setAdminFeeSgd] = useState(30);

  const result = useMemo(
    () =>
      estimateSchoolCcaKitBond({
        mode,
        bondSgd,
        overdueDays,
        overduePerDaySgd,
        damageSgd,
        adminFeeSgd,
      }),
    [mode, bondSgd, overdueDays, overduePerDaySgd, damageSgd, adminFeeSgd],
  );

  return (
    <div className="grid gap-10 lg:grid-cols-[1fr_20rem]">
      <fieldset className="space-y-8">
        <legend className="sr-only">School CCA kit bond inputs</legend>

        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint">
            Return path
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {MODES.map((m) => (
              <button
                key={m.id}
                type="button"
                onClick={() => setMode(m.id)}
                className={
                  mode === m.id
                    ? "bg-canopy px-4 py-2 text-sm font-semibold text-paper"
                    : "border border-ink/15 px-4 py-2 text-sm font-medium text-ink hover:border-ink/35"
                }
              >
                {m.label}
              </button>
            ))}
          </div>
        </div>

        <div className="grid gap-6 sm:grid-cols-2">
          <div>
            <label
              htmlFor="cca-bond"
              className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint"
            >
              CCA / instrument bond (SGD)
            </label>
            <input
              id="cca-bond"
              type="number"
              min={0}
              step={50}
              value={bondSgd}
              onChange={(e) =>
                setBondSgd(Math.max(0, Number(e.target.value) || 0))
              }
              className="mt-3 w-full border border-ink/15 bg-paper-elevated px-4 py-3 text-ink outline-none focus:border-canopy"
            />
          </div>
          <div>
            <label
              htmlFor="cca-admin"
              className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint"
            >
              Admin / cleaning fee (SGD)
            </label>
            <input
              id="cca-admin"
              type="number"
              min={0}
              step={10}
              value={adminFeeSgd}
              onChange={(e) =>
                setAdminFeeSgd(Math.max(0, Number(e.target.value) || 0))
              }
              className="mt-3 w-full border border-ink/15 bg-paper-elevated px-4 py-3 text-ink outline-none focus:border-canopy"
            />
          </div>
          <div>
            <label
              htmlFor="cca-overdue-days"
              className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint"
            >
              Overdue days
            </label>
            <input
              id="cca-overdue-days"
              type="number"
              min={0}
              max={120}
              step={1}
              value={overdueDays}
              onChange={(e) =>
                setOverdueDays(Math.max(0, Number(e.target.value) || 0))
              }
              className="mt-3 w-full max-w-[8rem] border border-ink/15 bg-paper-elevated px-4 py-3 text-ink outline-none focus:border-canopy"
            />
          </div>
          <div>
            <label
              htmlFor="cca-overdue-rate"
              className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint"
            >
              Overdue fee / day (SGD)
            </label>
            <input
              id="cca-overdue-rate"
              type="number"
              min={0}
              step={5}
              value={overduePerDaySgd}
              onChange={(e) =>
                setOverduePerDaySgd(Math.max(0, Number(e.target.value) || 0))
              }
              className="mt-3 w-full border border-ink/15 bg-paper-elevated px-4 py-3 text-ink outline-none focus:border-canopy"
            />
          </div>
          <div>
            <label
              htmlFor="cca-damage"
              className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint"
            >
              Damage / missing parts (SGD)
            </label>
            <input
              id="cca-damage"
              type="number"
              min={0}
              step={25}
              value={damageSgd}
              onChange={(e) =>
                setDamageSgd(Math.max(0, Number(e.target.value) || 0))
              }
              className="mt-3 w-full border border-ink/15 bg-paper-elevated px-4 py-3 text-ink outline-none focus:border-canopy"
            />
          </div>
        </div>
      </fieldset>

      <aside className="border border-fog-soft bg-paper-elevated p-6">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint">
          Sketch
        </p>
        <p className="mt-4 text-sm font-semibold text-ink">{result.headline}</p>
        <dl className="mt-6 space-y-3 text-sm">
          <div className="flex justify-between gap-3">
            <dt className="text-ink-faint">Cash in hope</dt>
            <dd className="font-medium tabular-nums text-canopy">
              {formatSgd(result.cashInSgd)}
            </dd>
          </div>
          <div className="flex justify-between gap-3">
            <dt className="text-ink-faint">Cash out sketch</dt>
            <dd className="font-medium tabular-nums text-tungsten">
              {formatSgd(result.cashOutSgd)}
            </dd>
          </div>
          <div className="flex justify-between gap-3">
            <dt className="text-ink-faint">Net sketch</dt>
            <dd className="font-medium tabular-nums text-ink">
              {formatSgd(result.netSketchSgd)}
            </dd>
          </div>
          <div className="flex justify-between gap-3">
            <dt className="text-ink-faint">Overdue days</dt>
            <dd className="font-medium tabular-nums text-ink">
              {result.overdueDays}
            </dd>
          </div>
        </dl>
        <p className="mt-6 text-sm leading-relaxed text-ink-muted">{result.note}</p>
        <p className="mt-6 text-xs text-ink-faint">
          Reviewed {SCHOOL_CCA_KIT_BOND_LAST_REVIEWED}. Not school handbook
          advice — published return windows and replacement schedules control
          real cash.
        </p>
      </aside>
    </div>
  );
}
